# API 總覽

ConvertX-CN 提供兩種 API 服務方式，滿足不同的整合需求。

---

## 🆕 Web UI 內建 API（簡易使用）

Web UI 服務（端口 3000）內建簡易 API 端點，適合基本使用場景。

### 特色

- ✅ **開箱即用**：不需額外配置
- ✅ **與 Web UI 整合**：共用認證和設定
- ⚠️ **功能受限**：不支援完整的 Job 管理

---

## 🚀 RAS-API Server（推薦）

**從 v2.0.0 開始**，提供獨立的 Rust RAS-API Server（端口 7890），支援完整的 API 功能。

### 特色

- ✅ **完整功能**：引擎查詢、格式驗證、Job 管理
- ✅ **Swagger UI**：互動式 API 文件
- ✅ **Cold Start Ready**：格式預測開箱即用
- ✅ **高效能**：Rust 實作，低資源消耗
- ✅ **OpenAPI 3.0**：標準化 API 規格

### 快速開始

```bash
# 健康檢查（不需認證）
curl http://localhost:7890/api/v1/health

# 查詢引擎（不需認證）
curl http://localhost:7890/api/v1/engines

# 查詢格式支援（不需認證）
curl http://localhost:7890/api/v1/formats/pdf/targets

# 驗證轉換（不需認證）
curl -X POST http://localhost:7890/api/v1/validate \
  -H "Content-Type: application/json" \
  -d '{"source_format": "docx", "target_format": "pdf"}'

# Swagger UI
open http://localhost:7890/swagger-ui
```

👉 **詳細說明請參閱 [RAS API 完整使用文件](RAS-API完整使用文件.md)**

- 🔍 **智慧建議**：轉換失敗時推薦替代引擎
- 🛠️ **20+ 轉換引擎**：與 Web UI 共用完整轉換器

---

## 快速啟用

API Server 是**選用功能**，不影響 Web UI 使用。

### 啟用方式

```bash
docker compose --profile api up -d
```

### 服務端口

| 服務           | 端口 | 說明               |
| -------------- | ---- | ------------------ |
| Web UI         | 3000 | 網頁介面           |
| RAS-API Server | 7890 | REST API & Swagger |

---

## API 端點

### REST API

- **Base URL**: `http://localhost:7890/api/v1`
- **Swagger UI**: `http://localhost:7890/swagger-ui`
- **Content-Type**: `application/json` 或 `multipart/form-data`

### GraphQL API

- **Endpoint**: `http://localhost:7890/graphql` (規劃中)

> ℹ️ GraphQL API 在 v2.0.0 暫時停用，建議使用 REST API

---

## 認證

所有 API 請求（除健康檢查外）都需要 JWT Bearer Token：

```http
Authorization: Bearer <your-jwt-token>
```

### Token 結構

```json
{
  "sub": "user-id",
  "exp": 1234567890,
  "iat": 1234567890,
  "email": "user@example.com",
  "roles": ["user"]
}
```

> API Server 只負責驗證 JWT，不負責產生 JWT。Token 應由獨立的認證服務產生。

---

## 環境變數

| 變數            | 說明         | 預設值              |
| --------------- | ------------ | ------------------- |
| `RAS_API_HOST`  | 監聽地址     | `0.0.0.0`           |
| `RAS_API_PORT`  | 監聽埠       | `7890`              |
| `JWT_SECRET`    | JWT 驗證密鑰 | （需自行設定）      |
| `UPLOAD_DIR`    | 上傳目錄     | `./data/uploads`    |
| `OUTPUT_DIR`    | 輸出目錄     | `./data/output`     |
| `MAX_FILE_SIZE` | 最大檔案大小 | `524288000` (500MB) |

---

## 快速範例

### 健康檢查

```bash
curl http://localhost:7890/api/v1/health
```

回應：

```json
{
  "data": {
    "status": "healthy",
    "version": "2.0.0",
    "timestamp": "2026-01-23T10:30:00Z"
  }
}
```

### 列出引擎

```bash
curl http://localhost:7890/api/v1/engines
```

### 轉換檔案

```bash
curl -X POST \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{"source_format": "docx", "target_format": "pdf"}' \
  http://localhost:7890/api/v1/jobs
```

---

## 相關文件

- [API 端點詳細說明](端點.md)
- [API 規格文件](../../api-server/docs/API_SPEC.md)
- [架構說明](../../api-server/docs/ARCHITECTURE.md)
