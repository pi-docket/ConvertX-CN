...
一個使用 Rust 實作的 REST 檔案轉換 API 伺服器。

## 🎯 功能特色

- **REST API**: 提供完整的 REST API 介面
- **JWT 認證**: 所有 API 請求都需要 JWT Bearer Token 驗證
- **多引擎支援**: 整合多種轉換引擎（FFmpeg、LibreOffice、GraphicsMagick 等）
- **任務管理**: 非同步轉換任務，支援狀態查詢和結果下載
- **權限控制**: 基於 Scope 的細粒度權限控制

## 🏗️ 系統架構

```
API Server (Port 7890)
├─ Auth Layer (JWT 驗證)
├─ REST API (/api/v1/*)
│   ├─ /engines (引擎查詢)
│   ├─ /convert (檔案轉換)
│   └─ /jobs (任務管理)
└─ Backend Proxy → ConvertX Web UI (Port 3000)
```
