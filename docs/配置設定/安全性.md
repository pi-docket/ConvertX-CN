...
ConvertX-CN 2.0.0 版本起，**Web UI 和 RAS API Server 共用同一個 `JWT_SECRET`**。

這表示：

- Web UI 登入產生的 Token 可以直接用於 API 認證
- 無需維護兩套認證系統
- 部署時只需設定一次 `JWT_SECRET`
...
1. **從 Web UI Cookie**：登入後，瀏覽器 Cookie 中的 `auth` 值就是 Token
2. **檢視方式**：開發者工具 → Application → Cookies → 找到 `auth`
